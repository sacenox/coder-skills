...
- **Mirror testing.** You read the implementation and write tests
  that assert what the code *does* — including its bugs. Tests
  should assert what the code *should do*.
...
Human-written tests with *lower* coverage catch 2x more bugs.
...
1. **Derive expectations from the spec, not the code.** Before
   ...
   function *should* do from its name, docstring, types, calling
